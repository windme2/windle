...
interface SurvivalModeGameProps {
  gameMode: GameMode;
  category: Category;
  onGameComplete: (score: { correct: number; wrong: number; totalRounds: number; lives: number }) => void;
  onBack: () => void;
}

const SurvivalModeGame = ({ gameMode, category, onGameComplete, onBack }: SurvivalModeGameProps) => {
  ...
  const handleRoundComplete = useCallback((roundScore: { correct: number; wrong: number }) => {
    setScore(prevScore => {
      const newScore = {
      ...
      if (roundScore.wrong > 0) {
        setLives(prevLives => {
          ...
          if (newLives <= 0) {
            onGameComplete({ ...newScore, totalRounds: currentRound, lives: 0 });
            return 0;
          }

          // ตรวจสอบว่าจบ 20 รอบหรือยัง
          if (currentRound >= totalRounds) {
            onGameComplete({ ...newScore, totalRounds: currentRound, lives: newLives });
            return newLives;
          }
        ...
        if (currentRound >= totalRounds) {
          setLives(prevLives => {
            onGameComplete({ ...newScore, totalRounds: currentRound, lives: prevLives });
            return prevLives;
          });
          return newScore;
        }
  ...
  return (
    <div className="min-h-[600px] flex items-center justify-center">
      <div className="max-w-4xl w-full space-y-4">
        ...
        <div className="bg-gradient-to-r from-blue-500 to-cyan-400 rounded-xl p-4 text-white shadow-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                <span className="text-2xl">🏆</span>
                <div>
                  <div className="text-xs font-medium opacity-90">SURVIVAL MODE</div>
                  <div className="text-lg font-bold">Round {currentRound} / {totalRounds}</div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="text-sm font-bold uppercase">{currentDifficulty}</div>
              </div>
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="text-right">
                ...
                <div className="text-lg font-bold">{score.correct} / {totalRounds}</div>
              </div>
