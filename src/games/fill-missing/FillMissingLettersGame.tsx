...
import { getWordsByCategoryAndDifficulty, type Difficulty, type Category } from '@/constants/wordBank';
...
const FillMissingLettersGame = ({ difficulty, category, onGameComplete, onBack, totalRounds = 5 }: FillMissingLettersGameProps) => {
  ...
  const getWordsForDifficultyAndCategory = (diff: Difficulty, cat: Category) => {
    return getWordsByCategoryAndDifficulty(cat, diff);
  };

  const generateWordWithMissingLetters = (word: string, difficulty: Difficulty) => {
    let lettersToHide = 0;
  ...
  const selectNewWord = useCallback(() => {
    ...
    if (availableWords.length === 0) {
      ...
      const wordWithMissing = generateWordWithMissingLetters(selectedWord.word, difficulty);
  ...
  }, [userGuess, currentWord, toast, gameStatus, wrongAttempts, maxWrongAttempts, displayWord, currentRound, totalRounds, score, onGameComplete, selectNewWord]);
  ...
  useEffect(() => {
    ...
    const initializeGame = () => {
      const availableWords = getWordsForDifficultyAndCategory(difficulty, category);
      if (availableWords.length === 0) {
        console.warn('No words available for this category and difficulty');
        return;
      }
  ...
  const handleBackspace = () => {
    ...
    for (let i = guessArray.length - 1; i >= 0; i--) {
      if (guessArray[i] !== '_' && displayWord[i] === '_') {
        const newGuess = userGuess.substring(0, i) + '_' + userGuess.substring(i + 1);
        setUserGuess(newGuess);
        break;
      }
    }
      ...
      <div className="max-w-4xl w-full">
        <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            ...
            {totalRounds > 1 && (
              <div className="flex-1 text-center text-slate-600">
                <div className="text-lg font-medium">Round {currentRound} of {totalRounds}</div>
              </div>
            )}
          ...
          <div className="text-center text-slate-600 text-sm space-y-1">
            ...
            <div>Wrong attempts: {wrongAttempts}/{maxWrongAttempts}</div>
          </div>
          ...
          <div className="flex justify-center items-center gap-2 mb-8 px-4 overflow-x-auto">
            <div className="flex gap-2 min-w-fit">
              {userGuess.split('').map((letter, index) => renderLetterBox(letter, index))}
            </div>
          </div>
          ...
          {gameStatus === 'won' && (
            <div className="text-emerald-600 font-semibold text-xl text-center animate-fade-in">
              🎉 Perfect! Well done!
            </div>
          )}
          
          {gameStatus === 'lost' && (
            <div className="text-red-600 font-semibold text-xl text-center animate-fade-in">
