...
const WordDisplay = ({ userGuess, gameStatus }: WordDisplayProps) => {
  const renderLetterBox = (letter: string, index: number) => {
    ...
    if (wordLength <= 4) {
      ...
      sizeClasses =
        "w-8 h-8 sm:w-10 sm:h-10 md:w-12 md:h-12 text-sm sm:text-base md:text-lg";
    } else {
      sizeClasses =
        "w-7 h-7 sm:w-8 sm:h-8 md:w-10 md:h-10 text-xs sm:text-sm md:text-base";
    }
  ...
  return (
    <div className="flex justify-center items-center gap-1 sm:gap-2 mb-4 sm:mb-8 px-2 sm:px-4 overflow-x-auto">
